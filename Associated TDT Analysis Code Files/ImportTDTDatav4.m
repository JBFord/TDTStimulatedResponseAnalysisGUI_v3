function app=ImportTDTDatav4_Parallel(app)
...
if FilterFlag
    ...
    LP=app.Variables.LowPassFilter;
          ...
          h=waitbar(0,strcat('Importing LFPs: 0% Complete'));  
...
for iStim=1:app.Variables.NumStims
    ...
    if ImportMult
     ...
     waitbar((iStim/app.Variables.NumStims)*0.8,h,strcat('Importing LFPs,', num2str(100*iStim/app.Variables.NumStims),'% Complete'));
end
...
if DetectOutliers
        waitbar(0.8,h,strcat('Importing Complete, Detecting Outliers'));
...
if ~isempty(ExcludeChannels) | isfield(app.Variables.Flags,'BadChannels')
    ...
    if FilterFlag
        InterpolatedFilteredRaw=AppInterpolateChannels(app.Variables,app.Variables.FilteredRawAverageOverSweeps);
