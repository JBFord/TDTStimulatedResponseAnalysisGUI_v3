function BatchImportTDTDat(BatchVariables,hObject,eventdata,handles,parentFolder,BaseName)
...
for iStim=1:BatchVariables.NumStims
    ...
    %%%%%%%% Raw Data Parsing
    Raw=data.Raws.data;
    RawArtifactLocations=BatchFindArtifacts(BatchVariables,Raw,PostWindowIndices,Fs,SweepStartIndices,iStim);

    %     if iStim == 8
    %         keyboard
    %     end
    %Chop up into sweeps base on artifact timing
    [RawSweeps,RawSweepTime]=ParseTDTDatav2(Raw,BatchVariables.NumSweeps,TraceTime,PreWindowIndices,PostWindowIndices,RawArtifactLocations);
    ...
    if FilterFlag
        ...
        for jj=1:BatchVariables.NumChannels
            FilteredRaw(jj,:)=transpose(filtfilt2((Raw(jj,:)),HP,LP,Fs, 1));
        end

        FilteredRawArtifactLocations=BatchFindArtifacts(BatchVariables,FilteredRaw,PostWindowIndices,Fs,SweepStartIndices,iStim);
        [FilteredRawSweeps,FilteredRawSweepTime]=ParseTDTDatav2(FilteredRaw,BatchVariables.NumSweeps,TraceTime,PreWindowIndices,PostWindowIndices,FilteredRawArtifactLocations);
    ...
    if ImportBP60
        ...
        [BP60Sweeps,BP60SweepTime]=ParseTDTDatav2(BP60,BatchVariables.NumSweeps,TraceTime,PreWindowIndices,PostWindowIndices,RawArtifactLocations);
        ...
        if FilterFlag
            ...
            %Chop up sweeps
            [FilteredBP60Sweeps,FilteredBP60SweepTime]=ParseTDTDatav2(FilteredBP60,BatchVariables.NumSweeps,TraceTime,PreWindowIndices,PostWindowIndices,FilteredRawArtifactLocations);
    ...
    if ImportMult
        ...
        [MultSweeps,MultSweepTime]=ParseTDTDatav2(Mult,BatchVariables.NumSweeps,TraceTime,PreWindowIndices,PostWindowIndices,RawArtifactLocations);
        ...
        if FilterFlag
            ...
            %Chop up sweeps
            [FilteredMultSweeps,FilteredMultSweepTime]=ParseTDTDatav2(FilteredMult,BatchVariables.NumSweeps,TraceTime,PreWindowIndices,PostWindowIndices,FilteredRawArtifactLocations);
