...
Use the Select Windows tab to explore the data and define windows over which to analyze. Windows are specific to each channel and variable (LFPs or CSDs). The plot window will automatically update whenever a plot parameter or window extent is changed. Make sure to click "Accept Window" to add the window to memory. Once completed, click "Save Window Information" to write the windows to a .mat file and move on to analyzing the data.

Navigate to the Analyze tab. Select the streams for which analysis windows were created and the variables that you want to analyze. CSDs (current source densities) are the second spatial derivative over the 1-dimensional recording device and provide information about where, spatially, the source and sink of the electrical activity are located. Select the analyses to perform and then click the "Analyze" button. Once complete, results will be saved in a .mat file and a .csv file. All files output from this App will be saved in a folder created within the experiment folder called AnalysisGUIResults.

A full breakdown of the functionality of this App and how to control each field is described below.
