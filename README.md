...
Use the Select Windows tab to explore the data and define windows over which to analyze. Windows are specific to each data stream and channel. The plot window will automatically update whenever a plot parameter or window extent is changed. Make sure to click "Accept Window" to add the window to memory. Once completed, click "Save Window Information" to write the windows to a .mat file and move on to analyzing the data.
